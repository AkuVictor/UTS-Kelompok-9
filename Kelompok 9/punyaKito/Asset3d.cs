...
namespace punyaKito
{
    internal class Asset3d
    {
        ...
        public void renderK(int _lines, double time, Matrix4 temp, Matrix4 camera_view, Matrix4 camera_projection)
        {
            _shader.Use();
            GL.BindVertexArray(_vertexArrayObject);
            ...
            foreach (var item in Child)
            {
                item.renderK(_lines, time, temp, camera_view, camera_projection);
            }
        }

        public void renderV(Matrix4 camera_view, Matrix4 camera_projection)
        {
            _shader.Use();
            GL.BindVertexArray(_vertexArrayObject);
            ...
            {
                i.renderV(camera_view, camera_projection);
            }
        }

        public void renderR(Matrix4 camera_view, Matrix4 camera_projection)
        {
            _shader.Use();
            GL.BindVertexArray(_vertexArrayObject);
            ...
            foreach (var i in Child)
            {
                i.renderR(camera_view, camera_projection);
            }
        }
        ...
        public void createBoxVerticesBaru(float x, float y, float z, float length, float panjang, float lebar, float tinggi)
        {
            _centerPosition.X = x;
            _centerPosition.Y = y;
            _centerPosition.Z = z;
        ...
        public void createEllipsoid(float radiusX, float radiusY, float radiusZ, float _x, float _y, float _z)
        {
            ...
            for (float v = -pi; v <= pi; v += pi / 360)
            {
                for (float u = -pi / 2; u <= pi / 2; u += pi / 360)
                {
                    ...
                    temp_vector.Y = _y + (float)Math.Cos(u) * (float)Math.Sin(v) * radiusY;
                    temp_vector.Z = _z + (float)Math.Sin(u) * radiusZ;
                    _vertices.Add(temp_vector);
                }
            }
        }
        ...
        public void createTrapesium(float x, float y, float z, float length)
        {
            _centerPosition.X = x;
            _centerPosition.Y = y;
            _centerPosition.Z = z;
        ...
        public void createTrapesiumKAKI(float x, float y, float z, float length)
        {
            _centerPosition.X = x;
            _centerPosition.Y = y;
            _centerPosition.Z = z;
            ...
            _vertices.Add(temp_vector);
            //TITIK 3
            temp_vector.X = x - length / 1.1f;
            temp_vector.Y = y - length / 1.1f;
            temp_vector.Z = z - length / 1.1f;
            _vertices.Add(temp_vector);
            //TITIK 4
            temp_vector.X = x + length / 1.1f;
            temp_vector.Y = y - length / 1.1f;
